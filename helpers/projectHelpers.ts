...
export const getGroupProjects = async (groups: GroupApiType[]) => {
  try {
    const projectsPromises = groups.map(async (group) => {
      const projectResponse = await fetchAllPagesOfEntity<ProjectApiType>(
        apiEndpoint.get.groupProjects(group.id)
      );
  ...
  try {
    const projectsMembersPromises = projects.map(async (project) => {
      const projectMembersResponse =
        await fetchAllPagesOfEntity<ProjectMemberApiType>(
      ...
      return projectMembersResponse.data.map((member) => ({
        ...
        projects: [
          `${project.path_with_namespace} (${mapAccessLevelToAccessGroups(
            member.access_level
          )})`,
        ],
      }));
